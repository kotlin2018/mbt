...
func init(){
	...
	//第二步: 创建映射关系
	h := mbt.H{
		&activity: &BizActivity{}, // BizActivity{} 即:数据库中表biz_activity的实体类，这层映射表示: activity这个结构体对象对表 biz_activity 进行 CRUD操作!
	}
...
type TestService struct {
	it *ActivityDao //服务包含一个mapper操作数据库，类似java spring mvc
	//类似拷贝spring MVC的声明式事务注解
	//rollback:回滚操作为error类型(你也可以自定义实现了builtin.error接口的自定义struct，框架会把自定义的error类型转换为string，检查是否包含，是则回滚
	//tx:"" 开启事务，`tx:"required,error"` 指定传播行为为required(默认required))
	//tx这个Tag对应的事务:
	/*
		required: 表示如果当前事务存在，则支持当前事务。否则，会启动一个新的事务
		!required: 表示如果当前没有事务，就新建一个事务,否则返回错误。
		support: 表示如果当前事务存在，则支持当前事务，如果当前没有事务，就以非事务方式执行。
		!support: 表示以非事务方式执行操作，如果当前存在事务，则新建一个Session以非事务方式执行操作，把当前事务挂起。
		mandatory: 表示如果当前事务存在，则支持当前事务，如果当前没有事务，则返回事务嵌套错误。
		new: 表示新建一个全新Session开启一个全新事务，如果当前存在事务，则把当前事务挂起。

		never: 表示以非事务方式执行操作，如果当前存在事务，则返回事务嵌套错误。
		nested: 表示如果当前事务存在，则在嵌套事务内执行，如嵌套事务回滚，则只会在嵌套事务内回滚，不会影响当前事务。如果当前没有事务，则进行与Required类似的操作。
	*/
	// tx: nested 嵌套事务,即: UpdateName() UpdateRemark()中的方法,函数都是在嵌套事务中执行,都执行成功才一起commit(),但凡有一个执行失败,则整个事务Rollback()
	UpdateName   func(id string, name string) error   `tx:"nested" rollback:"error"`
	UpdateRemark func(id string, remark string) error `tx:"nested" rollback:"error"`
}

//第二种使用事务的方法: 嵌套事务/带有传播行为的事务(和java spring的类似，有传播行为)
func TestTestService(t *testing.T) {
	testService := initTestService()
	testService.UpdateName("221", "法师")
	testService.UpdateRemark("8","事务")
}
...
func initTestService() TestService {
	...
	testService = TestService{
		...
		UpdateRemark: func(id string, remark string) error {
			var ids = []string{"8", "9"}
			var activities, err = testService.it.SelectByIds(ids)
		...
		UpdateName: func(id string, name string) error {
			...
			testService.UpdateRemark("7", "p2")
			testService.UpdateRemark("8", "p1")
			return nil
		},
	}
	en.Tx(&testService)
